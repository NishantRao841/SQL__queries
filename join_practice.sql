...
select e.enrollment_id,e.course_name from enrollments as e;
...
select s.student_id,s.student_name,count(e.enrollment_id) from students as s
join enrollments as e on s.student_id=e.student_id group by s.student_id having count(e.enrollment_id) >2;
...
select char_length(student_name),count(e.enrollment_id) from students as s
join enrollments as e
